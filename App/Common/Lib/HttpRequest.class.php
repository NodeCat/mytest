...
class HttpRequest {
    ...
    public function __construct($url = "") {
        import("Common.Lib.HttpRequestPool");
        if (!empty($url)) {
            $this->set_url($url);
        }
    }
    ...
    public function set_debug($debug, $log_formatter = null) {
        $this->debug = $debug;
        if ($log_formatter != null) {
            if ($log_formatter instanceof Comm_Log_Formatter) {
                $this->log_formatter = $log_formatter;
            } else {
                throw new Exception('log_fomatter must be Comm_Log_Formatter');
            }
        }
    }
    ...
    public function curl_init() {
        ...
        $ch = HttpRequestPool::get_curl($this->get_host_id());
    ...
    public function get_curl_cli() {
        if (!$this->debug) {
            throw new Exception("cann't get info when debug disable");
        }
    ...
    public static function urlencode($input) {
        if (is_array($input)) {
            return array_map(array('HttpRequest', 'urlencode'), $input);
    ...
    public static function urlencode_raw($input) {
        if (is_array($input)) {
            return array_map(array('HttpRequest', 'urlencode_raw'), $input);
        } else if (is_scalar($input)) {
    ...
    public static function urlencode_rfc3986($input) {
        if (is_array($input)) {
            return array_map(array('HttpRequest', 'urlencode_rfc3986'), $input);
        } else if (is_scalar($input)) {
    ...
    private function load_referer(){
    	if (empty($this->referer)) {
        	...
        	$this->curl_cli .= " --referer ".$this->referer;
        }
    }
    
    public function set_http_referer($refer){
    	$this->referer = $refer;
    }
}