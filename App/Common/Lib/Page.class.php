...
class Page{
    ...
    private $config  = array(
        ...
        'first'  => '首页',
        'last'   => '尾页',
        'theme'  => '%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %AJAX%'
    );
    /**
     * 架构函数
     ...
     */
    public function __construct($totalRows, $listRows, $parameter = null, $target, $pagesId) {
        ...
        $this->jump_url = U(CONTROLLER_NAME.'/'.ACTION_NAME,$this->parameter);
    }
    /**
     * 定制分页链接设置
...
eco;
        ...
        $page_str = str_replace(
            array('%HEADER%', '%NOW_PAGE%', '%UP_PAGE%', '%DOWN_PAGE%', '%FIRST%', '%LINK_PAGE%', '%END%', '%TOTAL_ROW%', '%TOTAL_PAGE%', '%AJAX%'),
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages,$ajax),
            $this->config['theme']);
        return "{$page_str}";
    }
}